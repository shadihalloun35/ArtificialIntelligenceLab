...
class WSState:
    ...
    #W = np.random.rand(5)
    W = np.array([1.0,0.2,0.2,0.2,0.2])
    alpha = 0.001
    gamma = 0.98
    ...
    def Clone(self):
        ...
        st.size = self.size
        return st
    def Feature_Maping(self,playerjm):
        state = []
    ...
    def Estimated_V(self,playerjm):
        state = self.Feature_Maping(playerjm)

        return np.dot(np.array(self.W) , np.array(state))
...
def Get_Best_Move_V(rootstate, itermax, verbose = False):
    """ Conduct a UCT search for itermax iterations starting from rootstate.
        Return the best move from the rootstate.
        Assumes 2 alternating players (player 1 starts), with game results in the range [0.0, 1.0]."""
    ...
    for i in range(itermax):
        ...
        # Rollout - this can often be made orders of magnitude quicker using a state.GetRandomMove() function
        while state.GetMoves() != []: # while state is non-terminal
            state.DoMove(print.choice(state.GetMoves()))
        '''
    ...
    return sorted(rootnode.childNodes, key = lambda c: c.visits)[-1].move # return the move that was most visited
def Get_Best_Move_TD(rootstate, itermax, verbose = False):
    """ Conduct a UCT search for itermax iterations starting from rootstate.
        Return the best move from the rootstate.
        Assumes 2 alternating players (player 1 starts), with game results in the range [0.0, 1.0]."""
    ...
    for i in range(itermax):
        ...
        while state.GetMoves() != []: # while state is non-terminal
            state.DoMove(random.choice(state.GetMoves()))
        #calculate V_t and v_t+1

        #current estimate of state S_t
        ...
        while node != None: # backpropagate from the expanded node and work back to the root node
            ...
            v_t_plus_1 = state.Estimated_V(node.playerJustMoved)
            reward = state.GetResult(node.playerJustMoved)
            v_t = state.Estimated_V(3 - node.playerJustMoved)
            if not state.GetMoves()==[]:
                theta = reward + state.gamma*v_t_plus_1 - v_t
            else:
                z_t = state.GetResult(node.playerJustMoved)
                theta = reward + state.gamma*z_t - v_t
            state.W = state.W + state.alpha * theta * np.array(state.Feature_Maping(node.playerJustMoved)) 
...
def Player_Move(player, rootstate , itermax = 100, verbose = False):
    if player == 'mcts':
        best_move = Get_Best_Move(rootstate = rootstate, itermax= itermax, verbose=verbose)
    elif player == 'mcts_v':
        best_move = Get_Best_Move_V(rootstate = rootstate, itermax= itermax, verbose=verbose)
    elif player == 'mcts_td':
        best_move = Get_Best_Move_TD(rootstate = rootstate, itermax = itermax, verbose=verbose)
    return best_move
def MCTSPlayGame(pair):
    ...
    while (state.GetMoves() != []):
        ...
        if state.playerJustMoved == 1:
            best_move = Player_Move(pair[1],rootstate = state, itermax = 100, verbose = False) # play with values for itermax and verbose = True
        else:
            best_move = Player_Move(pair[0],rootstate = state, itermax = 100, verbose = False)
    ...
    print('Complete after',steps,'steps', 'winer is',who_win)
    who_loose = pair[1] if who_win == pair[0] else pair[0]
    print ('looseer',who_loose)
    return who_win, who_loose
...
if __name__ == "__main__":
    ...
    for _ in range(N_SIMULATIONS):
        for pair in player_pair:
            print(pair[0],pair[1])
            who_win,who_loose = MCTSPlayGame(pair)
            wins[who_win] += 1
            looses[who_loose] += 1

    for player in ['mcts','mcts_td','mcts_v']:
        print('player: ',player,wins[player],looses[player])
