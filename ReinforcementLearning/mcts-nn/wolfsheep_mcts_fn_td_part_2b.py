...
def MCTSPlayGame():
    ...
    while (state.GetMoves() != []):
        ...
        if state.playerJustMoved == 1:
            best_move = Get_Best_Move(rootstate = state, itermax = 100, verbose = False) # play with values for itermax and verbose = True
        else:
            best_move = Get_Best_Move(rootstate = state, itermax = 100, verbose = False )
