...
def MCTSPlayGame():
    ...
    while (state.GetMoves() != []):
        ...
        if state.playerJustMoved == 1:
            ...
            best_move = Get_Best_Move(rootstate = state, itermax = 1000, verbose = False )
...
def get_action(state):
    move = None
    wolf, sheeps, _ = state.get_characters()
    print('TURN',3 - state.playerJustMoved)
    for s in sheeps:
    ...
    try:
        ...
        for i,s in enumerate(sheeps):
            if s.pos_x == _x and s.pos_y == _y:
                move = WolfSheepMove(i,_x,_y, x, y, 3 - state.playerJustMoved)
...
def MCTSPlayHumanGame():
    ...
    while (state.GetMoves() != []):
        ...
        state.show_boad_game()
        if state.playerJustMoved == 1:
            best_move = get_action(state)#Get_Best_Move(rootstate = state, itermax = 10000, verbose = False )
        else:
...
if __name__ == "__main__":
    ...
    """
    option = input('you want to play auto? (y,n)')
    if option.lower() == 'y':
        MCTSPlayGame()
    else: 
        MCTSPlayHumanGame()
